...
const MainNav = ({ children }) => {
  ...
  useEffect(() => {
    window.addEventListener("resize", () => setWidth(menuRef.current.offsetWidth));
  }, []);
  ...
  return (
    <div>
      <div className='absolute z-50  '>
        <div
          ref={menuRef}
          className={`bg-primary/95 overflow-x-auto  p-6   h-screen flex flex-col relative  ${
            open ? ` w-[500px] ` : ` w-16`
          } duration-1000`}
        >
          <div className={`grid grid-cols-2  whitespace-nowrap items-center`}>
            ...
            <div className='flex justify-end'>
              <button
                className={` bg-gray-600 hover:bg-gray-500 px-3 py-2  rounded-md duration-300  ${!open && "scale-0"}  `}
              >
